...
import { createRfq, publishRfq, uploadRfqFile } from '../../../../services/rfq';
...
export default function CreateRfqPage() {
  ...
  // Load active vendors when reaching Step 3
  useEffect(() => {
  ...
  const handleSubmitRfq = async (shouldPublish: boolean = false) => {
    ...
    setSubmitting(true);
    try {
      // Build ISO string matching datetime
      ...
      if (res.success && res.data?.rfq) {
        const createdRfq = res.data.rfq;
        if (shouldPublish) {
          const publishRes = await publishRfq(createdRfq.id);
          if (publishRes.success) {
            toast.success('RFQ Created and Published successfully!');
          } else {
            toast.warning('RFQ Draft created, but failed to publish: ' + publishRes.error);
          }
        } else {
  ...
  return (
    <RoleGuard allowedRoles={['OFFICER']}>
      <div className="max-w-3xl mx-auto space-y-6 font-sans text-[#212529]">
        {/* Header */}
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-[#212529]">Create Request for Quotation</h1>
          <p className="text-sm text-[#6b7280] mt-1">Onboard suppliers, declare materials required, and request price bidding.</p>
        </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1 col-span-2">
                  ...
                  <textarea
                    required
                    rows={4}
                    placeholder="Declare technical specification parameters, GST invoice conditions, delivery conditions, and L1/L2 approval benchmarks..."
                    value={description}
            ...
            <div className="space-y-4">
              ...
              {loadingVendors ? (
                ...
                <div className="py-8 text-center text-xs text-[#8F8F8F] border border-dashed border-[#e5e5e5] rounded-[6px]">
                  No active suppliers available. Verify supplier directory statuses.
                </div>
              ) : (
